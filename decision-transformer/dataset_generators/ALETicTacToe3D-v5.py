...
env = gym.make("ALE/TicTacToe3D-v5")
...
action_dtype = np.int64 if hasattr(env.action_space, 'dtype') else np.int64 

with h5py.File('TicTacToe3D-v5.h5', 'w') as f:
    ...
    def flush_to_disk():
        ...
        episode_frame_number_arr = np.array(buffer_episode_frame_number, dtype=np.int32)
        frame_number_arr = np.array(buffer_frame_number, dtype=np.int32)
        ...
        d_episode_frame_number[current_size:new_size] = episode_frame_number_arr

        d_frame_number.resize((new_size,))
        d_frame_number[current_size:new_size] = frame_number_arr
